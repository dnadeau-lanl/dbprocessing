...
#======================= Main ===========================================
input_path = "/projects/senser/data/rfs/rtm/"
sage_flists = os.listdir(input_path)
#scratch_path = "/projects/sdn/scratch/"
scratch_path = "/projects/sdndev/scratch/rfs/rtm/"
...
input_files = sorted(glob.glob(os.path.join(input_path, os.path.basename(input_file[0])[:4]+"*VC33*[0-9]")))
...
for each_sage in input_files:
    ...
    if not os.path.isfile(each_sage):    # skip a directory
        continue
    sage_time = re.findall(r'\d{13}', os.path.basename(each_sage))
    year = sage_time[0][0:4]
    doy  = sage_time[0][4:7]
    hms  = sage_time[0][7:]     # hms: hour, minute, second
    year_mon_day = convert_year_doy2_date(int(year), int(doy))

    ubet_mon = year_mon_day.month
    if ubet_mon < 10 :
        ubet_mon = '0' + str(ubet_mon)

    ubet_day = year_mon_day.day
    if ubet_day < 10 :
        ubet_day = '0' + str(ubet_day)

#    removed_vcdu_file = scratch_path + 'senser_xxxx_' + str(year) + str(ubet_mon) + str(ubet_day) + '_' + hms + '.removed_vcdu_hdr'
    removed_vcdu_file = scratch_path + 'senser_xxxx_' + str(year) + str(ubet_mon) + str(ubet_day) + '_' + hms + '_SERN_VC33.1.0.0.rtm'
    #removed_vcdu_file = args.files[0].split(' ')[1]
    ...
    if os.path.isfile(ubet_fileName):
       ...
       print ubet_fileName, " overwite"
       os.remove(ubet_fileName)
#       continue
#       sys.exit(0)

...
class ProcessQueue(object):
    ...
    def __init__(self,
                 mission='Test', instrument="1", engine='sqlite', dryrun=False, echo=False):

        self.dryrun = dryrun
        self.mission = mission
        self.instrument = instrument
        if engine == 'psql':
            mission = os.environ['PGDATABASE']
        self.tempdir = None
        dbu = DButils.DButils(mission, instrument, echo=echo, engine=engine)  #
        self.runme_list = []
        self.dbu = dbu
    ...
    def moveToError(self, fname):
        ...
        if os.path.islink(fname):
            ...
            DBlogging.dblogger.info(
                "moveToError file {0} was a link, so link removed not moved to error".format(fname))
        else:
            try:
    ...
    def diskfileToDB(self, df):
        ...
        try:
            if not self.dryrun:
                ...
                DBlogging.dblogger.info(
                    "File {0} entered in DB, f_id={1}".format(df.filename, f_id))
            else:
    ...
    def importFromIncoming(self):
        ...
        DBlogging.dblogger.debug(
            "Entering importFromIncoming, {0} to import".format(len(self.queue)))
        ...
        for ii, val in enumerate(vals, 1):
            ...
            DBlogging.dblogger.debug("popped '{0}' from the queue: {1} left".format(
                self.basename, len(self.queue)))
            # see if the file is in the db, if so then don't call the inspectors
            try:
                id = self.dbu.getFileID(self.basename)
                # Added 2019-02-11 XYang, do update if file is in db with different checksum
                if self.dbu.checkIncomingFileSHA(id):
                    DBlogging.dblogger.info(
                        'File {0}:{1} was already in DB, not inspecting'.format(id, self.basename))
                    self.moveToError(self.filename)
                    T1 = time.time() - T0
                    print('{1}:{2} Removed from incoming: {0} - already present  {3:.2f}s'.format(
                        self.basename, ii, len(self.queue), T1))
                    T0 = time.time()
                    continue
                else:
                    ...
                    T1 = time.time() - T0
                    print(
                        '{1}:{2} Removed from incoming: {0} - ingested  {3:.2f}s'.format(self.basename, ii, len(self.queue), T1))
                    T0 = time.time()
    ...
    def figureProduct(self, filename=None):
        ...
        for code, arg, product in act_insp:
            try:
                ...
                if os.path.isfile(code + ' '):
                    DBlogging.dblogger.info(
                        '---> However inspector: "{0}" was found'.format(code + ' '))
            ...
            if arg is not None:
                ...
                try:
                    ...
                    DBlogging.dblogger.error(
                        "File {0} inspector threw an exception".format(filename))
                    continue  # try the next inspector
            if df is not None:
                claimed.append(df)
        ...
        if len(claimed) > 1:
            DBlogging.dblogger.error(
                "File {0} matched more than one product, there is a DB error".format(filename))
    ...
    def _getRequiredProducts(self, process_id, file_id, utc_file_date, debug=False):
        ...
        T0 = time.time()
        input_product_id = self.dbu.getInputProductID(
            process_id)  # this is a list of tuples (id, optional)
        if debug:
            print("21:    {0}: self.dbu.getInputProductID: {1}".format(
                time.time() - T0, input_product_id))
        T0 = time.time()
        ...
        if debug:
            print("22:    {0}: self.dbu.getProcessTimebase: {1}".format(time.time() - T0, timebase))
        T0 = time.time()
        ...
        if timebase in ['FILE', 'DAILY', 'RUN']:  # taking one file to the next file
            ...
            raise (NotImplementedError(
                'Not implemented yet: {0} based processing'.format(timebase)))
    ...
    def buildChildren(self, file_id, debug=False):
        ...
        if debug:
            print("11:   {1}: done self.dbu.getChildrenProcesses buildChildren: file_id={0} : {2}".format(
                file_id, time.time()-T0, children))
        T0 = time.time()
        daterange = self.dbu.getFileDates(file_id[0])  # this is the dates that this file spans
        if debug:
            print("12:   {1}: done self.dbu.getFileDates  buildChildren: file_id={0} : {2}".format(
                file_id, time.time()-T0, daterange))
        T0 = time.time()
        ...
        for child_process in children:
            ...
            for utc_file_date in Utils.expandDates(*daterange):
                if debug:
                    print("    utc_file_date: {0}".format(utc_file_date))
                files, input_product_id = self._getRequiredProducts(
                    child_process, file_id[0], utc_file_date)
                ...
                # exit if the same process ....
                if self.dbu.dbprocessCurrentlyrunning(child_process):
                ...
                # Added check on process_id to ....
                if child_process in [p.process_id for p in self.runme_list] and timebase == 'DAILY':
                ...
                if runme.ableToRun and (runme not in self.runme_list):
                    ...
                    DBlogging.dblogger.info(
                        "Filename: {0} is not in the DB, can process".format(runme.filename))
